...
6. **连贯镜头判定（frameType=first 时）**：读取目标镜头 `custom_data.continuous`（分镜生成时已标记）：
   - **为 true（连贯镜头）**：首帧**不调用 generate_image**，直接调用 `extract_video_frame(videoUrl=上一镜头的 generatedVideoUrl, frame=last)` 提取上一镜头视频的真实尾帧。
   - 若上一镜头还没有视频（generatedVideoUrl 为空），退回第 7 步用 generate_image 生成首帧。
   - 未标记或为 false 的镜头：按普通流程用 generate_image 生成首帧。
